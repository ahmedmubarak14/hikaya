...
/**
 * Creator card on /discover with a favorite heart button overlay.
 * Photo-first, no dark gradient overlay — the disciplines + meta live below
 * the image so the work breathes uncovered.
 */
export function CreatorCardWithFavorite({ creator, isFavorited }: Props) {
  ...
  return (
    <div className="group relative flex flex-col gap-3">
      <Link href={`/${locale}/${creator.username}`} className="flex flex-col gap-3">
        <div className="bg-surface/5 relative aspect-[4/5] w-full overflow-hidden rounded-xl">
          ...
          <div className="absolute end-2 top-2 z-10 opacity-0 transition-opacity group-hover:opacity-100 focus-within:opacity-100">
            <FavoriteButton
