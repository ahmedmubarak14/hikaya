...
export function DiscoverHero({ title, placeholder, searchLabel, onSearch }: Props) {
  ...
  return (
    <section className="mx-auto flex max-w-3xl flex-col items-center gap-6 pb-10 text-center md:pb-12">
      <h1 className="text-surface text-balance text-4xl font-bold tracking-tight md:text-5xl lg:text-6xl">
        {title}
      </h1>
      ...
      <form
        ...
        className={cn(
          'border-surface/15 bg-bg relative flex h-14 w-full items-center rounded-full border pe-2 ps-5',
          'shadow-[0_1px_0_rgba(0,0,0,0.02),0_8px_24px_-12px_rgba(0,0,0,0.18)]',
          'focus-within:border-surface/40',
        )}
      >
        <SearchIcon className="text-surface/40 me-3 h-5 w-5 shrink-0" />
        <input
          type="search"
          value={q}
        ...
        <button
          type="submit"
          className={cn(
            'bg-surface text-bg ms-2 inline-flex h-10 shrink-0 items-center justify-center rounded-full px-6 text-sm font-medium',
