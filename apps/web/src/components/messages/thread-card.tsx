    ...
    <Link href={`/${locale}/me/messages/${thread.id}`} className="block">
      <Card interactive className={cn(unreadCount > 0 && 'border-accent/40 bg-accent/5')}>
        <CardBody className="flex items-center gap-4 p-5">
          <span
            aria-hidden
            className="bg-accent text-ink grid h-12 w-12 shrink-0 place-items-center rounded-full text-xl font-semibold"
          >
            {otherInitial}
          </span>
          <div className="min-w-0 flex-1">
            <div className="flex items-baseline justify-between gap-3">
              <span className="text-surface truncate text-base">{otherName}</span>
              {thread.lastMessageAt ? (
                <span className="text-2xs text-surface/40 shrink-0">
                  {formatRelative(thread.lastMessageAt, locale)}
                </span>
              ) : null}
            </div>
            <p className="text-surface/60 line-clamp-1 text-sm">{preview ?? t('noPreview')}</p>
          </div>
          {unreadCount > 0 ? (
            <span className="bg-accent text-2xs text-ink grid h-6 min-w-6 shrink-0 place-items-center rounded-full px-1.5 font-mono">
              {unreadCount}
            </span>
          ) : null}
        </CardBody>
