...
import {
  inviteSpaceTeamMemberAction,
  removeSpaceTeamMemberAction,
...
interface Props {
  locale: Locale;
  spaceId: string;
  members: SpaceTeamMember[];
  isOwner: boolean;
}

export function SpaceTeamManager({ locale, spaceId, members, isOwner }: Props) {
  const t = useTranslations('spaces.team');
  ...
  const handleInvite = () => {
    ...
    startTransition(async () => {
      const result = await inviteSpaceTeamMemberAction(locale, spaceId, email, role === 'admin');
      if (result.ok) {
        setSuccess(true);
        setEmail('');
      } else {
  ...
  const handleRemove = (memberId: string) => {
    startTransition(async () => {
      await removeSpaceTeamMemberAction(locale, spaceId, memberId);
    });
  };
